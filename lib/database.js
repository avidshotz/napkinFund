...
export const pdfsService = {
  ...
  async unpassPdf(pdfId, userId) {
    ...
    try {
      ...
      // Find ALL connections for this combination to debug
      ...
      console.log('  - All connections for this VC/founder/idea:', allConnections)
      
      // Find the blocked connection specifically
      const { data: connection, error: connectionError } = await supabase
        .from('connections')
        .select('id')
        .eq('vc_id', userId)
      ...
      console.log('  - Found blocked connection:', connection)
      
      if (connection) {
        console.log('  - Changing blocked connection back to curious status')
        
        // Change status to 'unpassed' so it doesn't show up in any lists
        const { data: updateData, error: updateError } = await supabase
          .from('connections')
          .update({ status: 'unpassed' })
          .eq('id', connection.id)
          .select()
        ...
        console.log('  ✅ Successfully changed status to unpassed:', updateData)
      } else {
        console.log('  ⚠️ No blocked connection found to unpass')
      }
