...
// Simple cache to store auth state across requests to prevent rate limiting
let sessionCache = {
  hasValidSession: false,
  lastChecked: 0,
  cacheExpiry: 1000 // 1 second cache to prevent rapid successive calls
}

// Check if user has valid session with caching to prevent rate limits
async function hasValidSession() {
  const now = Date.now()
  
  // Return cached result if recent
  if (now - sessionCache.lastChecked < sessionCache.cacheExpiry) {
    return sessionCache.hasValidSession
  }
  
  try {
    const { data: { session } } = await supabase.auth.getSession()
    sessionCache.hasValidSession = !!session
    sessionCache.lastChecked = now
    return sessionCache.hasValidSession
  } catch (error) {
    console.warn('Session check failed:', error)
    sessionCache.hasValidSession = false
    sessionCache.lastChecked = now
    return false
  }
}

// Function to invalidate session cache (call on sign out)
export function invalidateSessionCache() {
  sessionCache.hasValidSession = false
  sessionCache.lastChecked = 0
}
...
export const pdfsService = {
  ...
  async getAllPdfs() {
    ...
    try {
      // Verify we have a valid session before making database calls (cached)
      if (!(await hasValidSession())) {
        console.warn('No valid session found, skipping database query')
        return []
      }
      ...
      // Debug specific idea (remove after testing)
      // const targetIdea = data?.find(pdf => pdf.id === '0b1d2230-a5ed-467d-9837-7cb87506d777');
      // if (targetIdea) console.log('🔍 Found target idea in getAllPdfs:', targetIdea);
  ...
  async getLikedPdfsByUser(userId) {
    ...
    try {
      ...
      // Get connections where user is the VC and status is curious, pending, requested, or connected (but not passed)
  ...
  async getFounderLikedPdfs(founderId) {
    ...
    try {
      // Get connections where founder is the creator and status is pending (founder has liked back, but not passed)
      const { data: connections, error: connectionsError } = await supabase
        .from('connections')
  ...
  async passPdf(pdfId, userId) {
    ...
    try {
      ...
      // Check if there's already any connection for this VC/founder/idea
      const { data: existingConnection, error: connectionError } = await supabase
        .from('connections')
        .select('id, status, is_passed')
        .eq('vc_id', userId)
        .eq('founder_id', founderId)
        .eq('idea_id', pdfId)
        .maybeSingle()
      ...
      if (!existingConnection) {
        // Create a new connection marked as passed
        console.log('Creating new connection marked as passed')
        const { data, error } = await supabase
          .from('connections')
          .insert([
            {
        ...
        if (error) throw error
        console.log('Created pass tracking connection:', data?.[0])
        return data?.[0] || null
      } else if (!existingConnection.is_passed) {
        // Mark existing connection as passed (preserves original status)
        console.log('Marking existing connection as passed, preserving status:', existingConnection.status)
        const { data, error } = await supabase
          .from('connections')
        ...
        if (error) throw error
        console.log('Updated connection to passed:', data?.[0])
        return data?.[0] || null
      } else {
        console.log('Connection already marked as passed')
        return existingConnection
      }
    } catch (error) {
  ...
  async unpassPdf(pdfId, userId) {
    ...
    try {
      console.log('🔄 unpassPdf called with pdfId:', pdfId, 'userId:', userId)
      ...
      // Find the passed connection for this VC/founder/idea combination
      const { data: connection, error: connectionError } = await supabase
        .from('connections')
        .select('id, status, is_passed')
        .eq('vc_id', userId)
      ...
      console.log('  - Found passed connection:', connection)
      
      if (connection) {
        console.log('  - Unmarking connection as passed to restore original state')
        console.log('  - Original status will be restored:', connection.status)
        
        // Simply unmark as passed - this restores the original connection state
        ...
        if (updateData && updateData.length > 0) {
          console.log('  ✅ Successfully unmarked as passed:', updateData)
          console.log('  ✅ Original connection state restored with status:', updateData[0].status)
        } else {
          console.log('  ⚠️ Update returned empty result')
        }
      } else {
        console.log('  ⚠️ No passed connection found to unpass')
      }
  ...
  async getPassedPdfsByUser(userId) {
    ...
    try {
      ...
      // Get connections where user is the VC and is_passed is true
      const { data: connections, error: connectionsError } = await supabase
        .from('connections')
        .select('*, idea:idea_id (*, creator:user_id (name, link, vcphotourl, vcphotolink))')
        .eq('vc_id', userId)
      ...
      console.log('Found connections with is_passed=true:', connections?.length || 0)
      
      // Debug: Check if target idea is still in passed list (remove after testing)
      // const targetConnection = connections?.find(conn => conn.idea_id === '0b1d2230-a5ed-467d-9837-7cb87506d777');
      // if (targetConnection) console.log('🚫 Target idea still in passed list');
  ...
  async getVCLikedIdeasByVC(currentFounderId) {
    ...
    try {
      // Get connections where the founder is the creator and status is pending, requested, or connected (founder has reciprocated, but not passed)
      const { data: connections, error: connectionsError } = await supabase
        .from('connections')
